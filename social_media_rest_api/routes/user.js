...
router.put("/:id/follow", async (req, res) => {
    if (req.body.userId !== req.params.id) {
        try {
            const user = await User.findById(req.params.id); // another user
            const curruntUser = await User.findById(req.body.userId); // self user
            // in another user array put current user id into followers array.
            /*
                    parth(currentUser) -> FOLLOWERS -> body.userId
                    /    |        \
                MOHAN()  VARUN()   MADHAV() -> FOLLOWINGS -> parmas.id(parameters)
           */
